...
public class Main
{
    ...
    public static void main(String[] args)
    {
        ...
        do
        {
            ...
            cmd = SafeInputt.getRegExString(in, "\nA - Add\nD - Delete\nP - Print\nQ - Quit\n","[AaDdPpQq]");
            cmd = cmd.toUpperCase(Locale.ROOT);

            switch (cmd)
            {
                case "A":
                    userInput = SafeInputt.getNonZeroLenString(in, "Enter item to add to the list");
                ...
                case "D":
                    if (myArrList.size() > 0)
                    {
                        deleteNum = SafeInputt.getRangedInt(in, "Enter the item you wish to delete: ", 1, myArrList.size());
                ...
                case "Q":
                    ...
                    quitList = SafeInputt.getYNConfirm(in, "\nAre you sure you want to quit?");
                    if (quitList)
                    {
